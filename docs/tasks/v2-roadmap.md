...
| H-009 | Deploy to Base Mainnet + verify — **owner-gated** (funded keystore, BaseScan key, D-112). Prep done 2026-09-12: full suite green locally against the live fork (238/0/8), no-key fork simulation of the deploy ran (bits `0x28C0`, ≈8.16M gas), `deploy/dynamic-market/deploy.sh` wraps preflight → dry run → confirm → broadcast. **Deployed 2026-09-23** (hook `0xb23d3EeC…028c0`, stack + periphery in the runbook); hook BaseScan verification pending one `verify.sh` rerun | 🟡     |
...
> Foundation: PR #61 (`3b2c329`); the five read/context services: PR #62;
> docs/ledger: PR #63; OpenAPI publication + catalog↔spec parity test:
> PR #66.
...
| MP-005 | Market discovery — `GET /api/x402/v1/markets/discover`, $0.001, payment                                                                                                                            | ✅ PR #62 — `x402-discovery.ts` (+ tests)                                                                                                           |
| MP-006 | Market intelligence — `GET /api/x402/v1/intelligence/market`, $0.01, payment                                                                                                                       | ✅ PR #62 — `x402-intelligence.ts` (+ tests)                                                                                                        |
...
| MP-009 | Portfolio & exposure — `GET /api/x402/v1/portfolio/exposure`, $0.005, payment (public chain state only)                                                                                            | ✅ PR #62 — `x402-portfolio.ts` (+ tests)                                                                                                           |
| MP-010 | Hedging plans — `GET /api/x402/v1/hedging/plan`, $0.01, payment (arms nothing; B9-004 discipline)                                                                                                  | ✅ PR #62 — `x402-hedging.ts`, `lib/x402/hedge-templates.ts` (+ tests)                                                                              |
| MP-011 | Sports intelligence pilot — `GET /api/x402/v1/sports/context`, $0.01, allowlist + payment (pre-settlement refusal; empty allowlist = dark)                                                         | ✅ PR #62 — `x402-sports.ts` (+ tests)                                                                                                              |
...
| 17 — Circle Agent Marketplace + Agent-Native Services       | 🟢 P2 | MP-001 … MP-011 | 🟡 (task 073; foundation #61 + services #62 + docs/ledger #63 + OpenAPI #66 landed; go-live counsel-gated) |
