...
>
> Snapshot: 2026-09-12 (post-066) · **21 ✅ · 0 🟡 · 0 ⬜**.
> Lanes 062 – 066 all landed: activity spine · position economics ·
> timeline + chain-branding sweep · portfolio surfaces · end-to-end +
> the valuation decision (D-116).
...
| ------ | -------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| PF-001 | §1 Assets — everything held across wallets/accounts                                                            | ✅     | "Everything you hold" aggregate across wallet, agent wallet, unified balance, market positions and liquidity, naming unreadable sources (065)        |
| PF-002 | §2 Positions (liquidity) — per pool deposited, value, fees, P&L, share                                         | ✅     | `lp-economics.ts` + `/api/portfolio/economics` (063) rendered by `LpEconomicsSection` (065): deposited, value, fees, P&L, share                      |
| PF-003 | §3 Market positions — event, market, side, entry, current, value, unrealized P&L, potential payout, status     | ✅     | Grouped by game with league, group value and payout; per row entry, unrealized P&L, "pays $X if it wins" (063 + 065)                                 |
| PF-004 | §4 Agent — active/inactive, strategy, spending limit, current activity, permissions, recent actions            | ✅     | `AgentStatusStrip` (status, per-trade ceiling, leagues, unprompted permission, track record) + policy panel + recent actions from the timeline (065) |
| PF-005 | §5 Agent positions — sports positions, hedges, other authorized trades                                         | ✅     | Agent tab: the agent wallet's market positions (same section, agent address) + LP positions (065)                                                    |
| PF-006 | §6 Unified balance                                                                                             | ✅     | C-008 (`UnifiedBalanceTab`)                                                                                                                          |
| PF-007 | §7 Earnings — LP earnings plus other realized earnings                                                         | ✅     | Accrued LP fees (B6) + realized market P&L / win rate; LP fees collected stated as not tracked (063 + 065)                                           |
| PF-008 | §8 User wallet — wallet, assets, transactions                                                                  | ✅     | Wallet, assets, and the Activity tab as the transaction record (064 + 065)                                                                           |
| PF-009 | §9 Liquidity positions — detailed per-pool LP view                                                             | ✅     | Per-pool LP view in the Positions tab and on the profile (065)                                                                                       |
| PF-010 | §10 Hedging — hedge relationship, value, performance                                                           | ✅     | Hedges carry `position_ref` (062); the strategies section shows each executed hedge's value and market (065)                                         |
| PF-011 | Real-time valuation, never hardcoded                                                                           | ✅     | Pool marks for markets; Pyth-first + DefiLlama fallback for assets (D-116); a $0 valuation is counted, logged and alerted (`pricing_zero`) — 066     |
| PF-012 | Settled-position history with realized P&L                                                                     | ✅     | `/api/portfolio/settled` (063) rendered by `SettledPositionsSection` (065): W/L/void, cost · sold · paid · realized, claimed / claimable             |
| PF-013 | E2E: every section renders real data                                                                           | ✅     | `portfolio-e2e.test.ts` (066): one composed user, every section from the routes' wire shapes                                                         |
...
| PF-021 | E2E: user trade, agent trade, hedge, settlement each appear                                                    | ✅     | `activity-e2e.test.ts` (066): user trade, agent trade, hedge, settlement, Circle send pending → completed, through the shipped writers               |
