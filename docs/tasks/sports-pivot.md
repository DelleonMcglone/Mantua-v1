...
| B10-005 | Agent E2E: natural language → parse → preview → confirm → execute → audit entry — ✅ the full chain is live: NL → parse → preview/confirm → execute (agent wallet) → audit with tx hash; each link carries its own tests                                                                                                                                                             | 🔴 P0    | ⏸      |
| B10-006 | Hedging E2E: arm strategy → trigger fires → executes under cap → disarms on freeze — ✅ decision layer test-proven AND the execute-under-cap leg live (`strategy-execute.ts`: balance clamped by strategy cap, agent-wallet signed)                                                                                                                                                  | 🟠 P1    | ⏸      |
