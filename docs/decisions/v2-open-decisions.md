...
| D-106 | x402 payments — scope, non-goals, gate                                                                                     | Build gate satisfied (C-wave landed); Phase 17 built the six-service seller catalog + dual-rail paywall with tests and roadmap rows (PR #61 foundation + PR #62 services + PR #63 docs/ledger + PR #66 OpenAPI publication, all merged); open questions (b)/(c) resolved 2026-09-19; go-live counsel-gated (D-012)                                                                                                                                                                                                                                     | High (facts)                                                | Counsel (D-012 seller-revenue posture gates go-live)                                                     |
...
- **(b) List more services beyond the analyst brief? — YES.** Six services
  ...
  five read/context services shipped in PR #62 (+26 tests); the OpenAPI
  publication + catalog↔spec parity test shipped in PR #66 (seven
  hand-authored OpenAPI 3.1 documents, served unpaid, plus a services
  index).
...
- ~~Whether the shipped surfaces get roadmap rows + tests or stay
  ...
  section) and tests (PR #61's suite; the catalog↔spec parity test shipped
  in PR #66).
...
## D-119 — Combos: a combo is a market ✅ CLOSED 2026-09-19

**Decision.** A combo (task 072, CB-001) is a conjunction market: a new
full-collateral market whose YES pays $1 if every leg's YES pays, created
through the existing `MarketFactory` with an id derived from the sorted
leg market ids (`computeComboMarketId`), priced by its own YES/USDC pool
under the Dynamic Market Hook, bought and sold with the same single
`PoolSwapTest` swap every game trade uses, and resolved by the existing
`Resolver` from the legs' on-chain outcomes. Legs are the YES sides of
existing moneyline markets; the ticket (`combos` / `combo_legs`) is the
user's record of one buy of that market.

**Reasoning.**

1. **The only construction that pays like a parlay with the deployed
   contracts.** A bundle of independent positions pays the sum of its
   winning legs (the holder keeps them), and sequencing legs by
   resolution time fails for same-slot games — the owner's own example.
   A conjunction token has the product payoff by definition.
2. **Everything else falls out.** One swap is CB-005; the hook's own
   `quoteFee` is CB-004's transparency standard; one token balance marked
   at one pool price is CB-008; `Resolver.resolve` is CB-007; the agent's
   trade path is CB-006. No batching, no Permit2, no new signer.
3. **No Solidity.** The factory takes any `bytes32` id, `resolve`
   accepts any market it created, `voidMarket` covers the all-void case,
   and `startsAt` = the latest leg kickoff keeps the 12 h backstop past
   every leg. Foundry is also unavailable in the build session, and code
   that cannot be compiled and tested is not shipped.
4. **Honest pricing.** Fair probability is the product of the legs'
   pool prices, disclosed as such; the pool quotes the price actually
   paid and the premium over fair is shown. Correlation is not modelled:
   two legs from one game or one team twice are refused in code
   (`combo-rules.ts`), never priced.
5. **Settlement follows the legs.** Each leg's result is stamped from its
   own resolution (criteria gate and dispute window already passed); the
   combo resolves through a dedicated authorization mint whose evidence
   is exactly those results. A lost leg settles the combo NO as soon as
   the resolver may freeze it (its `startsAt`); a void leg drops out; all
   void → INVALID, $0.50 a share as on any market (B4-005).

**Rejected.** A combo desk contract with a house reserve (a new audited
contract and a counterparty model, for a P2 feature). Sequential
"rolling" stakes (no payout to lock, impossible for simultaneous games).
A basket of independent legs called a combo (pays the sum, not the
product — the user would be misled). Pricing correlation with a model
(no source of truth; refusal is honest, a guessed haircut is not).

**Consequence.** Server: `server/src/lib/combos/` (rules, pricing, policy
gate, settlement rule, monitor, proposal, drift, on-chain plan/build,
store, quote, fill, tickets, runners), `computeComboMarketId`, migration
0024, `routes/combos.ts`, `routes/combos-trade.ts`, `routes/cron-combos.ts`,
the resolution cron's combo pass, the `combo` policy block, the
`mantua_build_combo` / `mantua_execute_combo` tools and the `combo`
preview kind. Client: `client/src/features/combos/` (builder, positions
section, limits form), `+ Combo` on every game row, the Combos nav entry.
Operator: `COMBO_MAX_LEGS`, `COMBO_MAX_STAKE_USDC`, `COMBO_MAX_OPEN_MARKETS`,
`COMBO_SEED_USDC`; the combo market creation and seeding ride the sync's
own `createMarketsOnChain`. Follow-ups: per-combo liquidity depth (a thin
combo pool trades wide — the premium line says so), correlation across
markets of one game once totals/spreads ship (DM-106), combo tickets on
the public performance ledger (Phase 13 reads `market_fills`).
...
spend. The qualified custodian holding the institution's principal is
recorded, not integrated: its deposit addresses form the withdrawal
allowlist, verified by a second person, and a withdrawal at or above the
institution's threshold is approved by a second person. Institution-wide
per-trade and aggregate daily caps sit on top of each wallet's cap. All
of it is enforced at the three choke points money already passes —
`checkSpendingCap`, `sendFromAgentWallet`, `getOrCreateAgentWallet`.

**Why.** Circle exposes exactly one segregation primitive to this stack —
the wallet set — and it is the unit its per-set controls (Gas Station
policy, transaction screening) and balances attach to. Building on it
keeps the custody posture of C-009 (entity secret in the secrets manager,
Privy for people) and adds no key, dependency or contract.

**Alternatives rejected.**

- **Circle user-controlled / modular wallets for institutional users** —
  D-110's non-goal; a login-stack migration for no gain in segregation.
- **A custodian API integration (Fireblocks, BitGo, …)** — a second
  credential and a second custody model in the server for the
  institution's principal, which the custodian already governs; Mantua's
  job is the trading balance.
- **One shared institutional wallet** — loses per-member attribution and
  the existing per-wallet cap and ledger; per-member wallets in one set
  keep both.

**Owner items.** `MANTUA_OPS_KEY` for the operator routes; in the Circle
Console, a Gas Station policy for each institution's wallet set and
transaction screening on it.

## D-121 — Home page restructure: one front door, board-first when logged out ✅ CLOSED 2026-09-20

**Decision.** The standalone marketing `LandingPage` (task 075, HP-002)
is deleted; `/` resolves straight to the app's `home` route — the board
of today's games — for every visitor, logged in or not. A logged-out
visitor sees the board immediately, with no interstitial and no login
wall; the login gate stays exactly where it already was, at the point
of trade (B5-007), not in front of browsing. The landing page's footer
content that must keep working — the Documentation link, the social
channels, the copyright line, and the Privacy/Terms of Use/Market
Integrity links the Terms acceptance gate depends on (L-004/L-005) —
moves verbatim into a new footer on the home page; the marketing
content above it (hero art, demo video, feature grid, FAQ) is dropped,
recorded block-by-block in `docs/tasks/075-home-page-restructure.md`'s
content inventory.

**Why.** This is not a new behavior choice so much as a recorded one:
`home` already worked fully logged out before this change (proven by
`errors.spec.ts`'s pre-existing "logged out: browsing works and the
ticket asks to log in" test), reachable only via one extra click
through the landing page's "Launch App" button. Removing that click
makes the already-correct behavior the first thing every visitor sees,
and turns an implicit assumption into a decision with a test pinned to
it (the same spec, strengthened to assert no "Launch App" affordance
exists any more).

**Alternatives rejected.**

- **A gated shell at `/`** (board hidden behind a login wall) — rejected
  outright; it contradicts B5-007 (browsing is free) and would be a
  regression from the shipped, tested behavior one click away.
- **Keep a slimmed marketing page in front of home** — rejected; the
  owner's instruction is to delete the standalone page, not shrink it,
  and there is no launch requirement asking for marketing copy ahead of
  the product.

**Owner items.** None — no env var, no operator step. The marketing
assets (hero PNGs, demo video) are left in `client/public/assets/`
unreferenced, in case a future marketing surface wants them; they cost
nothing sitting there (static files, not bundled JS).
