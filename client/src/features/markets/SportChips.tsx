 ...
 * marked `soon` is listed and labelled, and opens the coming-soon page —
 * never a live market.
 */
      ...
      {SPORTS.map((s) => {
        ...
        return (
          <button
            ...
            className={`inline-flex min-h-11 shrink-0 items-center gap-1.5 rounded-full border px-4 text-[13px] font-medium transition-colors cursor-pointer ${
              active === s.id
                ? "border-accent bg-accent/15 text-text"
                : "border-border-soft bg-chip text-text-dim hover:text-text"
            }`}
          >
            <LeagueLogo league={s} className="h-4 w-4" />
            {s.label}
            {soon && (
